...
#appears in n genomes and accounts for contig coverage. List of KOs needs one KO entry per line and output from kofamscan. 
#Script will output KO counts in a table. Example KO list in git repo="Nitrogen_list"

#Usage: python3 kegg_parser.py path/to/kofamsccan/output Nitrogen_list
        ...
        with open(os.path.join(input_directory, kegg)) as input:
                for line in input:
                        if len(line.split("\t")) == 1: #if there is no KO annotation for this gene, ignore it
                                ...
                                if ko in genome_dict:
                                        genome_dict[ko] = genome_dict[ko]+cov
                                else:
                                        pass
        input.close()
